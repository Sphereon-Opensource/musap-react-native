...
function noKMSTestRun(sscdInfo: SscdInfo) {
    ...
    console.log(`NOKMS Generating key \n\n`)
    ...
    MusapModule.generateKey('TEE', keyGenRequest, (error: any, keyUri: string) => {
        if (sscdInfo.sscdName === "SE" && error) {
            ...
            console.log(error)
            console.log(`NOKMS GetKeyByUri(): ${MusapModule.getKeyByUri(error)}`)
        } else {
            if (error) {
                console.log(error)
            }
...
function kmsTestRun() {
    ...
    async function generateKey() {
        ...
        const data = encoder.encode(JSON.stringify(jwt));
